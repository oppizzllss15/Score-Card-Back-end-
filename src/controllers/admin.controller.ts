...
const createAdmin = asyncHandler(async (req: Request, res: Response) => {
  ...
  const validation: ValidationResult = await adminRegistrationSchema.validateAsync(req.body);
  ...
  const isUserInRegistered: IAdmin = await isPropertyInDatabase("email", req.body.email)
